...
def main():
    ...
    for data_dict in tqdm.tqdm(all_data_dict[_start_:_end_]):
        ...
        question = ("All spatial relationships are defined from the viewer’s perspective, where ‘front’ means closer to the viewer and ‘back’ means farther from the viewer. This is a referring-expression segmentation task. First decide whether the image visibly contains the object or all objects matching every detail in the statement. This benchmark contains many no-target queries: if no matching object is visible, if the description is ambiguous, or if only a similar object is present, do not guess and output exactly null with no mask tokens. If the described object or all described instances are clearly present, provide exactly one complete segmentation mask group covering them in the format <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|>. Output only null or that one mask group; do not output explanations, extra mask groups, or a mask for a rejected query. Statement: " + phrase)
