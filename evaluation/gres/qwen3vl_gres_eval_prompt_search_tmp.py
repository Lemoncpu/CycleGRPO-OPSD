...
def main():
    ...
    for data_dict in tqdm.tqdm(all_data_dict[_start_:_end_]):
        ...
        variant = os.environ.get("PROMPT_VARIANT", "strict_match")
        prompt_prefix = "All spatial relationships are defined from the viewer's perspective, where 'front' means closer to the viewer and 'back' means farther from the viewer. "
        variants = {
            "strict_match": prompt_prefix + "This is a referring-expression segmentation task. Check the image against every detail in the statement, including object category, attributes, count, spatial relations, and actions. A match is valid only when all details refer to the same visible object or requested set. If any required detail is absent or conflicts with the image, output exactly No target. If all details are visibly satisfied, output the complete segmentation mask in the format <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|>. Output only No target. or the mask, with no explanation.",
            "evidence_check": prompt_prefix + "First verify each noun, attribute, number, spatial relation, and action in the statement against visible image evidence. Do not infer an object from a similar object or from a partial match. Output exactly No target. when even one required detail is not visibly supported. Otherwise output the complete mask of the matching object or set in the format <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|>. Output only No target. or the mask.",
            "conservative_match": prompt_prefix + "Only segment an object when the image provides clear visual evidence for every part of the statement and no part is contradictory. Partial, approximate, ambiguous, or similar matches are No target. If and only if the complete statement matches, output its mask in the format <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|>; otherwise output exactly No target. Output no explanation.",
            "target_safe": prompt_prefix + "Inspect the whole statement carefully. If a clearly matching object or set is visible, always provide its mask, even when the object is small or there are other similar objects. If the exact object, attribute, relation, count, or action is not visibly present, output exactly No target. Output only the mask in the format <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> or No target., never both.",
            "two_stage": prompt_prefix + "Decide in two steps: (1) confirm that every described object, attribute, relation, count, and action is visible; (2) only then localize the matching object or set. If step 1 fails, output exactly No target. If step 1 succeeds, output the complete mask in the format <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|>. Do not guess from a similar object and do not add explanations.",
            "strict_cardinality": prompt_prefix + "Check every detail in the statement against visible evidence. A partial, approximate, ambiguous, or similar match is No target. If all details match, output every requested object: use one complete mask group in the format <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> for each distinct object or instance named by the statement, preserving the requested count and never merging, omitting, or replacing instances. If any required detail is absent, output exactly No target. Output only No target. or the complete mask groups, with no explanation.",
            "concise_cardinality": "Please segment the object or objects described by this statement in this image. Match the complete statement exactly. If no object satisfies all details, output exactly No target. If a match exists, use the learned mask format and output one complete mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> for every requested object or instance; never merge or omit multiple objects. Output only No target. or the complete mask groups, with no explanation.",
            "count_and_evidence": "Please segment the object or objects described by this statement in this image. Verify that the described category, attributes, relations, and actions are visibly supported; if the image has no matching object, output exactly No target. Minor spelling or grammar errors in the statement do not invalidate a clearly visible match. Count every object joined by 'and', 'both', 'two', or 'three' and output one complete mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> for each requested object; never merge, duplicate, or omit instances. Output only No target. or the complete mask groups, with no explanation."
            ,"visual_tolerance": "Please segment the object or objects described by this statement in this image. Use visual evidence to match the requested objects; minor spelling, grammar, small size, partial occlusion, or awkward relation wording does not invalidate a clearly visible object. For every object named by 'and', 'both', 'two', or 'three', output one complete mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> and do not merge or omit instances. Output exactly No target. only when no requested object is visibly present or the complete request is contradicted by the image. Output only No target. or the complete mask groups, with no explanation."
            ,"multi_tolerant": "Please segment the object or objects described by this statement in this image. For a single object, require its category and all distinguishing attributes to be visibly supported; if not, output exactly No target. For a request containing 'and', 'both', 'two', or 'three', verify that every named object is visibly present and output one complete mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> for each, but do not reject the complete visible set because a relation phrase is awkward or grammatically noisy. Never merge or omit instances. Output No target. only when the requested object(s) are absent or contradicted, and otherwise output only the complete mask groups.",
            "dynamic": "Please segment the object or objects described by this statement in this image. Match the complete statement against visible evidence. If no matching object or complete requested set is present, output exactly No target. If a match exists, output one complete mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> for each requested object or instance; never merge or omit objects. Output only No target. or the complete mask groups, with no explanation.",
            "dynamic_relaxed": "Please segment the object or objects described by this statement in this image. Match the requested object categories and visible attributes. If no requested object is visible, output exactly No target. If requested objects are visible, output one complete mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> for each requested object or instance; preserve all instances and do not merge or omit them. Use visual evidence rather than rejecting a visible object because a multi-object relation phrase is awkward. Output only No target. or the complete mask groups, with no explanation."
            ,"dynamic_attr_strict": "Please segment the object or objects described by this statement in this image. Match every requested object category and distinguishing attribute to visible evidence. If any named category or attribute is absent, contradictory, or only approximately matched, output exactly No target. If all named objects and attributes are visible, output one complete mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> for each requested object or instance; preserve all instances and do not merge or omit them. Relation wording may contain minor grammar noise, but never use a similar object as a substitute. Output only No target. or the complete mask groups."
            ,"dynamic_balanced": "Please segment the object or objects described by this statement in this image. Identify every explicitly named object category and use visible evidence to decide whether it is present. If no named category has a matching visible object, output exactly No target. If the named categories are visible, output one complete mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> for each requested instance; preserve all instances and never merge or omit them. Treat minor relation, grammar, or wording noise as non-decisive, but do not substitute a different category. Output only No target. or the complete mask groups."
        }
        if variant not in variants:
            raise ValueError(f"Unknown PROMPT_VARIANT={variant}; choose from {sorted(variants)}")
        if variant in {"dynamic", "dynamic_relaxed", "dynamic_attr_strict", "dynamic_balanced"}:
            multi = bool(re.search(r"\b(and|both|two|three|four|pair|couple)\b", phrase))
            if multi:
                if variant == "dynamic_relaxed":
                    question = ("Please segment every requested object in this multi-object statement. "
                                "If the named object categories and visible attributes are present, output one complete "
                                "mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> for each instance; preserve all instances "
                                "and do not merge or omit them. Do not reject a visible object because a relation phrase is "
                                "awkward. If no requested object category is visible, output exactly No target. Output only "
                                "No target. or the complete mask groups. Statement: " + phrase)
                elif variant == "dynamic_attr_strict":
                    question = ("Please segment every requested object in this multi-object statement. Verify every named "
                                "object category and distinguishing attribute against the image. If any category or attribute "
                                "is absent or only approximately matched, output exactly No target. If all are visible, output "
                                "one complete mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> for each instance; preserve "
                                "all instances and never merge or omit them. Tolerate only minor relation grammar noise. "
                                "Output only No target. or the complete mask groups. Statement: " + phrase)
                elif variant == "dynamic_balanced":
                    question = ("Please segment every requested object in this multi-object statement. Identify each named "
                                "object category and check that it has a visually matching instance. If no named category is "
                                "visible, output exactly No target. If the named categories are visible, output one complete "
                                "mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> for every requested instance; preserve "
                                "all instances and never merge or omit them. Do not reject a visible category because relation "
                                "wording is awkward, but never replace it with a different category. Output only No target. or "
                                "the complete mask groups. Statement: " + phrase)
                else:
                    question = ("Please segment every object described by this multi-object statement in this image. "
                                "Verify that every named object is visibly present; tolerate only spelling or grammar noise, "
                                "not an absent object. Output one complete mask group <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|> "
                                "for each named object, without merging or omitting instances. If any named object is absent, "
                                "output exactly No target. Output only No target. or the complete mask groups. Statement: " + phrase)
            else:
                question = ("Please segment the object described by this statement in this image. "
                            "Require the category and all distinguishing attributes and relations to be visibly supported. "
                            "If no exact matching object is present, output exactly No target. If it is present, output its "
                            "complete mask in the format <|mt_start|><|mt_xxxx|><|mt_xxxx|><|mt_end|>. Output only No target. "
                            "or the mask. Statement: " + phrase)
        else:
            question = variants[variant] + " Statement: " + phrase
