#!/usr/bin/env bash
# Historical supervised baseline with every training stream reduced to 25%.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export RUN_NAME="${RUN_NAME:-cyclegrpo70k_historical_baseline_25pct_8gpu}"
...
export RAY_SHORT_ROOT="${RAY_SHORT_ROOT:-/dev/shm/cgrpo70k-25}"
