...
class BatchFunctionRewardManager(FunctionRewardManager):
    ...
    def compute_reward(self, step: int, data: DataProto, task: str) -> Tuple[torch.Tensor, dict[str, list[float]]]:
            ...
            reward_inputs.append(
                {
                    ...
                    "source": data.non_tensor_batch["source"][i],
                    "task": task,
                    "iou_scores": data.non_tensor_batch["iou_scores"][i] if data.non_tensor_batch["source"][i] in ['groundingme', 'denseworld_single', 'denseworld_multiple', 'refcoco_cycle', 'tg_multi_merged', 'dam_cyclegrpo', None] else None,
        ...
        for i, score in enumerate(scores):
            ...
            for key, value in score.items():
                reward_metrics[key].append(value)
            if task == 'caption' and data.non_tensor_batch["source"][i] in ['groundingme', 'denseworld_single', 'denseworld_multiple', 'refcoco_cycle', 'tg_multi_merged', 'dam_cyclegrpo', '', None]:
