...
def _process_multi_modal_data(
    ...
    if "videos" in multi_modal_data:
        ...
        for vi, video in enumerate(multi_modal_data["videos"]):
            ...
            frames, metadata = process_video(
                video, min_pixels, max_pixels, video_fps, return_video_metadata=True, nframes=nf
            )
...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        # Vision-related special tokens that the model may erroneously emit during
        # generation. If left in `response_ids`, they pollute downstream
        # input_ids and break the (n_video_tokens == n_video_features) invariant
        # in qwen3_vl._get_input_embeds. We strip them by replacing with pad.
        _vision_special_tokens = [
            "<|vision_start|>", "<|vision_end|>",
            "<|image_pad|>", "<|video_pad|>",
        ]
        _vision_ids = [tokenizer.convert_tokens_to_ids(t) for t in _vision_special_tokens]
        unk_id = getattr(tokenizer, "unk_token_id", None)
        self.vision_token_ids = [tid for tid in _vision_ids if tid is not None and tid != unk_id]
        ...
        if processor is not None:  # only VLMs have processor
            ...
            import inspect
            llm_init_params = set(inspect.signature(LLM.__init__).parameters.keys())

            if "disable_mm_preprocessor_cache" in llm_init_params:
                engine_kwargs["disable_mm_preprocessor_cache"] = True
    ...
    def update_sampling_params(self, **kwargs):
        ...
        for key, value in kwargs.items():
            ...
            if key == "eos_token_id":
                ...
                if not ids:
                    continue
                old_values["stop_token_ids"] = list(self.sampling_params.stop_token_ids or [])
                merged = list(dict.fromkeys((self.sampling_params.stop_token_ids or []) + list(ids)))
                self.sampling_params.stop_token_ids = merged
                continue
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        with self.update_sampling_params(**prompts.meta_info):
            ...
            # Sanitize: replace any vision-related special tokens the model may have
            # generated with pad_token_id, otherwise they get counted as <|video_pad|>
            # in compute_log_probs and trip the "tokens != features" assertion.
            if self.vision_token_ids:
                _vt = torch.tensor(self.vision_token_ids, device=response_ids.device, dtype=response_ids.dtype)
                response_ids = response_ids.masked_fill(torch.isin(response_ids, _vt), self.pad_token_id)
