...
### 隔离 PEGC 实验副本（2026-09-02）

`experiments/pegc_ablation_20260902/` 是不修改仓库根目录实现的实验副本。其
`tools/prepare_pegc_subsets.py` 按 `source` 分层、固定 seed 生成 cycle 4k、direct
8k、no-target 2k 和 DLC-QA 2k 子集，并按 `dam_source_id` 同步筛选 QA JSONL。五个
对照共享这些文件、SAMTok 初始化和训练参数；实验差异仅限 PEGC evidence gate、
hierarchical mask credit、adaptive task balance、CBBA 与 Refusal Credit 的副本实现。

两套可复现的 1/10、一 epoch 独立训练入口为 `tools/train_evidence_mask_credit_1of10_epoch1.sh` 和 `tools/train_cbba_1of10_epoch1.sh`。两者均为新服务器单命令入口：自动在物理 GPU7 部署本地 Llama 3.1 8B DLC-QA judge（`127.0.0.1:8007`），并使用 GPU0--6 进行 7 卡训练；不依赖预先启动 judge，也不调用占卡工具。

为隔离 no-target 识别模式，`tools/train_direct_grpo_only_baseline_1of10_3gpu.sh` 与 `tools/train_direct_grpo_only_refusal_credit_1of10_3gpu.sh` 提供四卡 3+1 对照：GPU0--2 训练、GPU3 启动本地 Llama，但关闭 DLC-QA、Mask Credit、direct mask CE 和其他 PEGC 模块，仅保留正样本/no-target direct GRPO；第二个入口额外开启 Refusal Credit。direct grounding warmup 固定从第 0 步生效，确保一 epoch 内实际参与优化。
...
2. `verl/trainer/main.py::main` 按“dataclass 默认值 -> YAML -> CLI 覆盖”合并配置，并初始化 Ray；隔离 PEGC suite 可通过 `RAY_LOCAL_NUM_CPUS` 限制本地 Ray CPU 资源，避免预启动过多 worker。
...
`worker.supervised_anchors.refusal_credit` 是独立于 `direct_mask_ce` 和 `worker.opsd.mask_credit` 的 no-target 拒识监督。开启后，trainer 仍从 standalone direct loader 取 `gres_no_target` parent，仅为每个原始 UID 构造一条 `grounding_query -> <answer>No target.</answer>` teacher-forcing batch；该 batch 只覆盖拒识文本 token，不写入正样本 mask token，也不启用层级 mask token 权重。其梯度通过 `accumulate_refusal_credit_gradients` 单独累积，日志使用 `supervised_anchors/refusal_credit_*` 命名，因此可以在 `DIRECT_MASK_CE_ENABLED=false`、`worker.opsd.mask_credit.enabled=false` 时单独比较。`loss_weight` 支持和 direct CE 相同的线性 warmup，默认关闭。

同一 direct no-target 的 `K=6` rollout group 会按原始 `sample_uid` 聚合 `mask_group_count>0`：日志同时记录完整六次 rollout 中全部输出 mask 的比例 `supervised_anchors/no_target_all_6_mask_rate`、全部为空的比例和二元同质比例 `supervised_anchors/no_target_6_rollout_homogeneous_rate`。不完整的 rank-trim group 不计入分母。该统计只观察 rollout，不改变 reward、advantage 或 direct GRPO；它用于判断 no-target 偏好是否在采样层面已经塌缩为一致的 mask 输出。
...
| `data_1of10/` | PEGC 十分之一数据契约：2k cycle、4k direct、1k no-target，以及按 `dam_source_id` 对齐的 1k DLC-QA parquet/JSONL；由既有隔离 1/5 数据确定性抽样生成 |
...
| `trainer/main.py` | CLI/YAML 配置合并、可选本地 Ray CPU 上限、Ray runner、主 CycleGRPO 与独立 RefCOCO/DLC-QA loader、worker/reward/trainer 组装 |
...
| `workers/fsdp_workers.py` | actor/ref/critic 构建，FSDP-vLLM 权重切换，多模态前处理，rollout 后 token/tIoU 评分，以及 regenerate/direct-mask CE/Refusal Credit 的独立梯度累积调用 |
...
| `workers/opsd/config.py` | pixel IoU、路由、caption safety、EMA teacher、regenerate、distillation 及 PEGC 三模块配置与边界校验 |
| `workers/opsd/cbba.py` | CBBA 的 cycle 置信度聚合与 self-supervised/supervised 分支缩放纯函数 |
...
| `workers/opsd/mask_iou.py` | 完整/合法 SAMTok group 解析和计数、层级 code-token credit、原始 GT 转换、共享 image embedding 的批量 `union`/`first_mask` 解码、尺寸恢复和像素 IoU |
| `workers/opsd/routing.py` | `R_Ci` 聚合、三路由边界、caption 特殊 token/JSON/长度安全检查、原始 GRPO 启用判定、packed mask context、GT/reconstruction teacher crop 构造、evidence gate 与泄漏过滤 |
...
### 2026-09-02 - 创建 PEGC 隔离实验副本与五分之一数据

- 代码：新增 `experiments/pegc_ablation_20260902/README.md`、`tools/prepare_pegc_subsets.py`，复制训练入口为 `projects/rl/qwen3vl_4b_pegc_ablation.sh`；仅副本会继续修改，仓库根目录实现不变。
- 行为：固定 seed=20260902 按 source 分层生成 cycle 4k、direct 8k、no-target 2k、DLC-QA 2k，并按 `dam_source_id` 对齐 QA JSONL。新增三个互斥实验开关：evidence gate 按 IoU/false-positive 证据调节 teacher JSD，mask credit 对 SAMTok 粗/细两层 code token 使用不同 CE 权重，adaptive balance 按各流样本量缩放梯度。四版保持相同 `108/216/54` batch、1 step、`SAVE_FREQ=1` 和 SAMTok 初始化；`tools/run_pegc_suite.sh` 使用物理 GPU 1、3 训练且关闭 DLC-QA judge。GPU0/2 的共享 Ray 任务不属于本实验，后四卡绝不使用。
- 验证：数据脚本成功生成 4000/8000/2000/2000 行子集及 2000 行 QA JSONL；新增模块纯函数断言通过（evidence gate=0.8/0.5、mask weights=[1,1.5,1,1,1]），副本 launcher/suite 通过 `bash -n`，全部修改文件通过 `py_compile`。1-step smoke 已完成模型加载但在双卡 FSDP worker 初始化时因系统连接/资源异常退出，未产生可归因的训练指标；原始工作区训练脚本未修改。

### 2026-09-02 - 在隔离副本实现 PEGC 三模块并建立四版 suite

- 代码：仅修改 `experiments/pegc_ablation_20260902/verl/workers/opsd/{config,routing,mask_iou,__init__}.py`、`verl/trainer/ray_trainer.py`、`verl/workers/actor/dp_actor.py`、副本 launcher；新增 `tests/test_pegc_modules.py`、`tools/run_pegc_suite.sh`、`tools/run_pegc_eval_suite.sh`、`tools/summarize_pegc_results.py`。
- 行为：evidence gate 将 teacher JSD sample weight 乘以基于 IoU 和 reconstruction-only false-positive 面积的证据系数；mask credit 在 direct mask CE 中识别连续 SAMTok codebook token，分别加权 coarse/fine 深度；adaptive balance 按 caption/segmentation stream 样本量以平方根比例缩放 caption 与 segmentation 梯度。三个开关默认关闭，suite 每次只开启一个。
- 验证：`py_compile`、两个模块纯函数断言、`bash -n` 均通过；1-step 双卡 smoke 在 FSDP worker 初始化时因系统连接/资源异常退出，未进入 optimizer step。根目录原始代码和 launcher 未修改。

### 2026-09-02 - 完善 PEGC 副本的 unittest 验证与评测入口

- 代码：修改隔离副本 `tests/test_pegc_modules.py`；新增 `tools/run_pegc_eval_suite.sh`。
- 行为：PEGC 纯函数测试改为仓库兼容的 `unittest` 测试发现格式；评测入口按四个固定 run 目录导出 HF checkpoint，并依次运行 RefCOCO、GroundingSuite、GRES 与 DLC-Bench，物理设备限制为 0--3。
- 验证：`PYTHONDONTWRITEBYTECODE=1 ... -m unittest -v tests.test_pegc_modules` 通过 2 项；评测脚本 `bash -n` 通过。由于前四卡当时被既有评测任务占用且 smoke FSDP worker 已异常退出，未启动新的训练或评测。
### 2026-09-02 - 清理 PEGC 中断 worker 并收敛为可完成的四版短跑

- 代码：修改隔离副本 `tools/run_pegc_suite.sh`；同步更新 `README.md` 与本节实验约束。
- 行为：中断原 36-step baseline（首步已完成但未到保存频率）后，清理本实验 orphan Ray worker；suite 固定物理 GPU 0-2（3 卡）、`108/216/54` batch、`MAX_STEPS=1`、`SAVE_FREQ=1`，关闭需要外部 judge 的 DLC-QA，四版仍只互斥开启一个 PEGC 模块。GPU0/2 检测到共享 Ray 外部占用，未执行破坏性 reset 或误杀。
- 验证：`bash -n tools/run_pegc_suite.sh`；`nvidia-smi` 确认本实验 worker 已退出且 GPU3 judge 已停止；defunct 条目仅剩 PID1 下不可再发送信号的孤儿记录。

### 2026-09-02 - 修正 PEGC suite 的变体输出隔离

- 代码：修改隔离副本 `tools/run_pegc_suite.sh`。
- 行为：每个 baseline/evidence-gate/mask-credit/adaptive-balance 运行现在写入独立的 `logs/pegc_<variant>_4k`，避免共享 `RUN_ROOT` 造成日志与 checkpoint 互相覆盖。
- 验证：`bash -n tools/run_pegc_suite.sh` 与 `git diff --check` 通过。

### 2026-09-02 - 避让共享 Ray 任务改用 GPU3 单卡执行 PEGC suite

- 代码：修改隔离副本 `tools/run_pegc_suite.sh` 与 `README.md`。
- 行为：检测到共享 Ray 集群重新占用 GPU0-2 后，suite 改为仅使用空闲物理 GPU3（`NUM_GPUS=1`），四版仍固定相同 1/5 数据、`108/216/54` batch、1 step 和 `SAVE_FREQ=1`，不启动 DLC-QA judge。
- 验证：`bash -n tools/run_pegc_suite.sh`、`git diff --check`；`nvidia-smi` 确认 GPU3 空闲，GPU0-2 外部 PID 未修改。

### 2026-09-02 - 隔离 PEGC suite 的 Ray 临时目录

- 代码：修改隔离副本 `tools/run_pegc_suite.sh`。
- 行为：suite 现在使用独立的 `/dev/shm/pegc-ray-gpu3` 作为 `RAY_SHORT_ROOT`，不再与服务器共享 `/dev/shm/cgrpo-ray-0` Ray 集群共用 raylet/socket；此前 baseline 在共享目录冲突下收到 `ActorUnavailableError`，未生成 checkpoint。
- 验证：`bash -n tools/run_pegc_suite.sh`、`git diff --check`；失败日志确认本地 Raylet 意外终止，GPU3 运行期间未出现 CUDA OOM。

### 2026-09-02 - 记录 PEGC 单卡重试的 Raylet 失败证据

- 代码：无训练逻辑修改；更新隔离副本实验记录。
- 行为：设置独立 `RAY_SHORT_ROOT=/dev/shm/pegc-ray-gpu012` 后，baseline 能完成模型初始化、rollout 和多轮 log-prob 计算，但本地 Raylet 在 step 0 中途退出，driver 报 `ray.exceptions.ActorUnavailableError`，因此没有 `global_step_1` checkpoint；GPU3 随后释放。
- 验证：`tests.test_pegc_modules` 2/2 通过，受影响 Python 文件和三个 shell 入口通过语法检查；`nvidia-smi` 确认失败后无本实验 worker。

### 2026-09-02 - 限制 PEGC 本地 Ray CPU 资源

- 代码：修改隔离副本 `verl/trainer/main.py` 与 `tools/run_pegc_suite.sh`。
- 行为：新增 `RAY_LOCAL_NUM_CPUS` 校验并传递给本地 `ray.init(num_cpus=...)`；PEGC suite 默认设为 32，避免按服务器 216 CPU 预启动大量 Python worker，降低 Raylet 在共享机器上的资源压力。多机/共享 Ray 地址路径不受影响。
- 验证：`python3 -m py_compile verl/trainer/main.py`、`bash -n tools/run_pegc_suite.sh`、`git diff --check`。

### 2026-09-02 - 在 GPU3 保活下恢复三卡 PEGC suite

- 代码：修改隔离副本 `tools/run_pegc_suite.sh` 与 `README.md`。
- 行为：GPU0-2 运行 3-rank PEGC suite（`108/216/54` batch 可整除），GPU3 单独运行用户指定的 power-hold worker，避免四卡中有一张长期空置；suite 使用 `/dev/shm/pegc-ray-gpu012` 和 32 个本地 CPU 资源。
- 验证：已重新读取根目录 `code.md`/`Agent.md`，并通过 `bash -n tools/run_pegc_suite.sh`、`git diff --check`。

### 2026-09-02 - 对齐 PEGC 评测 GPU 与 FSDP shard 数

- 代码：修改隔离副本 `tools/run_pegc_eval_suite.sh`。
- 行为：评测 suite 不再把 `NUM_GPUS=4` 和 `CUDA_VISIBLE_DEVICES=0,1,2,3` 写死；现在通过 `NUM_GPUS` 与 `EVAL_GPU_LIST` 传递拓扑，默认使用与本次 3-rank 训练 checkpoint 匹配的 GPU 0--2，避免加载器寻找不存在的 `model_world_size_4_rank_*.pt`。
- 验证：`bash -n tools/run_pegc_eval_suite.sh`；错误配置的评测进程已停止并清理，随后确认前四卡无本实验进程且无 zombie。

### 2026-09-02 - 修正 PEGC 训练指标汇总匹配

- 代码：修改隔离副本 `tools/summarize_pegc_results.py`。
- 行为：汇总器同时接受带/不带 `opsd/` 命名空间的 console 日志键，能够提取四版短跑的 IoU、JSD、evidence gate 与 adaptive balance 指标；mask-credit 未额外输出标量时保留其基础指标。
- 验证：重新扫描四个训练日志并生成 `logs/pegc_summary.json`；脚本通过 Python 编译检查。

### 2026-09-03 - 切换 PEGC 四版实验为十分之一数据并重训

- 数据：新增隔离目录 `data_1of10/`，由既有 1/5 子集按 source 分层、seed=`20260903` 再取 1/2，包含 `cycle_2k.parquet`（2,000）、`direct_4k.parquet`（4,000）、`no_target_1k.parquet`（1,000）及按 `dam_source_id` 对齐的 `qa_1k.parquet`/`qa_1k.jsonl`（各 1,000）。原 `data/` 1/5 文件保留不变。
- 行为：后续 baseline、evidence gate、mask credit、adaptive balance 四个版本统一使用上述 1/10 数据，保持 batch、辅助监督和模块互斥设置不变；主流 2,000/108 在当前 loader 的 drop-last 语义下对应完整 epoch 的 18 个 global steps，全部完成后才进入逐版评测。
- 验证：PyArrow source 计数与 QA join 校验通过；已停止不再符合目标的 1/5 baseline Ray 作业，确认无残留 zombie worker，GPU4--7 未触碰。

### 2026-09-03 - 使 PEGC 评测入口支持 1/10 最终 checkpoint

- 代码：修改隔离副本 `tools/run_pegc_eval_suite.sh`，新增 `RUN_TAG` 与 `EVAL_STEP` 覆盖参数。
- 行为：评测目录、FSDP checkpoint 和导出 HF 目录不再固定为 `4k/global_step_36`；例如 `RUN_TAG=1of10 EVAL_STEP=18` 会逐一读取四个 `pegc_<variant>_1of10` 的最终 checkpoint。默认值仍保持旧 4k 行为。
- 验证：`bash -n tools/run_pegc_eval_suite.sh` 通过；未改变评测脚本的 benchmark 顺序和指标协议。

### 2026-09-06 - 新增 CBBA 置信度分支平衡试验模块

- 代码：修改隔离副本 `verl/workers/opsd/config.py`、`__init__.py`、`ray_trainer.py`、`projects/rl/config.yaml`、`projects/rl/qwen3vl_4b_pegc_ablation.sh`、`tools/run_pegc_suite.sh`、`tools/run_pegc_eval_suite.sh` 与 `tests/test_pegc_modules.py`；新增 `verl/workers/opsd/cbba.py`。
- 行为：新增 `CBBA_ENABLED` 及 confidence/scale 参数。开启后，driver 使用当前 cycle batch 的真实 pixel-IoU `R_Ci` 均值计算 `cycle_scale` 与 `supervised_scale`：低置信 step 降低 cycle segmentation、regenerate、privileged JSD 的梯度并提高 direct grounding、direct mask-CE、DLC-QA 的锚点梯度；高置信 step 反向调整。CBBA 默认关闭，baseline 与原有三模块行为不变；主仓库入口不修改。
- 验证：待执行 `PYTHONDONTWRITEBYTECODE=1 python -m unittest -v tests.test_pegc_modules`、`py_compile`、`bash -n`、1/10 一 epoch 训练和完整 benchmark 评测。


### 2026-09-06 - 固化两套 1/10 一 epoch 训练入口

- 代码：新增隔离副本 `tools/train_evidence_mask_credit_1of10_epoch1.sh` 与 `tools/train_cbba_1of10_epoch1.sh`；更新本文件实验说明和模块清单。
- 行为：两个入口分别开启 evidence+hierarchical mask credit，或仅开启 CBBA；均固定 1/10 的 2k cycle、4k direct、1k no-target、1k DLC-QA 数据，1 epoch/17 global steps（7 卡训练；3 卡临时验证仍为 18 steps），使用 GPU0--2 训练并要求 GPU3 的本地 Llama 3.1 judge 已在 8007 健康运行。训练结束或中断时自动恢复 GPU0--2 power hold，不修改根目录主训练入口。
- 验证：两个脚本均通过 `bash -n`；未在本次变更中启动训练或评测。

### 2026-09-06 - 将两套训练入口改为 7+1 自包含单服务器启动

- 代码：修改隔离副本 `tools/train_evidence_mask_credit_1of10_epoch1.sh` 与 `tools/train_cbba_1of10_epoch1.sh`。
- 行为：两个脚本现在都使用 8 张物理卡：GPU0--6 运行 7 卡 FSDP/Ray trainer，GPU7 由脚本自动启动本地 Llama 3.1 8B vLLM judge；健康检查通过后自动进入 1/10、一 epoch（18 steps）训练，训练结束时只关闭本脚本启动的 judge，不执行 GPU 占卡清理。用户只需在新服务器执行一个脚本命令。
- 验证：两个脚本均通过 `bash -n`；确认 launcher 拓扑为 `CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6`、`NUM_GPUS=7`、`LOCAL_JUDGE_ENABLED=true`；未启动实际训练。

### 2026-09-06 - 修复自包含训练入口并恢复 8 卡默认拓扑

- 代码：修改隔离副本 `tools/train_evidence_mask_credit_1of10_epoch1.sh` 与 `tools/train_cbba_1of10_epoch1.sh`。
- 行为：默认配置恢复为 GPU0--6 训练、GPU7 Llama judge、`NUM_GPUS=7`、默认 `MAX_STEPS=18`；保留 `TRAIN_GPU_LIST`、`TRAIN_NUM_GPUS`、`LLAMA_GPU` 环境覆盖，仅用于当前四卡服务器的临时 3+1 验证。修复 vLLM API key 健康检查 401，并缩短 Ray 临时目录到 launcher 的 32 字符限制以内。
- 验证：evidence+mask-credit 已在当前服务器以 GPU0--2+GPU3、`MAX_STEPS=1` 完成 Ray/FSDP 训练并写出 `global_step_1`；CBBA 已通过 Llama、数据检查和 Ray/FSDP 初始化并进入训练，随后按用户要求中止。两个脚本均通过 `bash -n`。

### 2026-09-07 - 训练入口启用终端与文件双路日志

- 代码：修改隔离副本 `projects/rl/qwen3vl_4b_pegc_ablation.sh`、`tools/train_evidence_mask_credit_1of10_epoch1.sh` 与 `tools/train_cbba_1of10_epoch1.sh`。
- 行为：新增 `STREAM_LOG=true` 时使用 `tee` 将 launcher、Ray、FSDP、Hydra 和 Python traceback 同时输出到当前终端和 `RUN_LOG`；两个自包含入口默认启用该模式。原有 file-only 日志仍可通过 `STREAM_LOG=false` 保留。
- 验证：三个 shell 文件均通过 `bash -n`；确认训练入口仍调用 `python -m verl.trainer.main`，其本地单机 Ray 路径保持不变。

### 2026-09-07 - 在两个自包含入口显式固定探索分支工作目录

- 代码：修改 `tools/train_evidence_mask_credit_1of10_epoch1.sh` 与 `tools/train_cbba_1of10_epoch1.sh`。
- 行为：启动 branch launcher 前显式执行 `cd "${REPO_DIR}"` 并检查失败；因此从任意当前工作目录执行时，launcher 的相对 `config=projects/rl/config.yaml` 都解析到探索分支，不会误加载根目录配置。终端实时日志和文件日志双路输出保持开启。
- 验证：两个入口和 branch launcher 均通过 `bash -n`；未启动新的训练任务。

### 2026-09-07 - 修复 7 卡 DataProto batch 不可均分

- 代码：修改隔离副本 `tools/train_evidence_mask_credit_1of10_epoch1.sh` 与 `tools/train_cbba_1of10_epoch1.sh`。
- 行为：正式 7+1 拓扑显式使用 `ROLLOUT_BATCH_SIZE=112`、`DIRECT_BATCH_SIZE=224`、`CAPTION_QA_BATCH_SIZE=56`、`ACTOR_GLOBAL_BATCH_SIZE=112`，使主/direct/DLC-QA 三条 parent batch 都能被 7 个 Ray worker 均分；1/10 的 2,000 条主数据在 drop-last 下对应 17 steps。此前的 `108` 只能被 3 卡整除，导致 8 卡启动时 `DataProto size 108 and chunk 7`。
- 验证：两个脚本通过 `bash -n`；根据实际 traceback 完成静态整除校验，未启动新的训练任务。

### 2026-09-07 - 新增 no-target Refusal Credit 对照路径

- 代码：修改隔离副本 `verl/workers/supervised_anchors.py`、`verl/workers/config.py`、`verl/workers/fsdp_workers.py`、`verl/trainer/main.py`、`verl/trainer/ray_trainer.py`、`projects/rl/config.yaml` 与 `projects/rl/qwen3vl_4b_pegc_ablation.sh`；新增 `tools/train_supervised_direct_grpo_baseline_1of10_3gpu.sh` 和 `tools/train_refusal_credit_1of10_3gpu.sh`。
- 行为：新增默认关闭的 `worker.supervised_anchors.refusal_credit`。开启时只对 gRefCOCO no-target 行 teacher-force `<answer>No target.</answer>`，作为独立拒识梯度；不启用 `direct_mask_ce`、层级 `mask_credit` 或正样本 mask CE。direct no-target 的六次 rollout 还按 `sample_uid` 统计全部输出 mask、全部为空和六次二元同质率，统计不改变 reward/advantage。分支 launcher 的本地 judge 拓扑允许 3 张训练卡，保持正式 7+1 约束不变。
- 训练入口：两个新脚本固定 `data_1of10`、`108/216/54`、`K=6`、1 epoch/18 steps、GPU0--2 训练和 GPU3 Llama；baseline 关闭 Refusal Credit，实验组只打开 Refusal Credit，二者都关闭 direct mask CE/Mask Credit。
- 模块清单：新增上述两个训练入口；Refusal Credit 复用 `ray_trainer.py` 的 teacher-forcing batch builder，新增 worker endpoint 和配置字段，无独立目录。
- 验证：`PYTHONDONTWRITEBYTECODE=1 /volume/ybo/xyc/envs/cyclegrpo/bin/python3 -m py_compile` 覆盖 5 个受影响 Python 文件；两个 launcher 与两个训练脚本均通过 `bash -n`。尚未启动本次一 epoch 训练或四 bench 评测。

### 2026-09-07 - 增加 Mask Credit + Refusal Credit 联合对照入口

- 代码：新增隔离副本 `tools/train_mask_refusal_credit_1of10_3gpu.sh`，复用已验证的 3+1 卡本地 Llama、Ray 和 1/10 数据配置。
- 行为：第三个实验固定 `DIRECT_GROUNDING_ENABLED=true`、`DIRECT_MASK_CE_ENABLED=false`、`MASK_CREDIT_ENABLED=true`、`REFUSAL_CREDIT_ENABLED=true`，用于比较 direct GRPO 上叠加 Mask Credit 与 no-target Refusal Credit 的联合效果，不混入 direct mask CE 或 evidence gate。运行名为 `pegc_mask_refusal_credit_1of10_epoch1`。
- 模块清单：新增上述训练入口。
- 验证：脚本通过 `bash -n`；实际训练将在前两版完成后启动，三版随后统一四 bench 评测。

### 2026-09-08 - 增加干净 direct GRPO 与 Refusal Credit 对照入口

- 代码：新增 `tools/train_direct_grpo_only_baseline_1of10_3gpu.sh` 与 `tools/train_direct_grpo_only_refusal_credit_1of10_3gpu.sh`。
- 行为：两套入口均固定 1/10 的 cycle、direct positive 和 no-target 数据，关闭 DLC-QA、Mask Credit、direct mask CE、evidence gate、adaptive balance 与 CBBA，仅使用 direct GRPO；direct grounding 从第 0 步生效。第二套入口只额外打开 no-target Refusal Credit，并使用独立 run root，避免与此前混入 DLC-QA 的历史实验混淆。
- 验证：两个脚本已通过 `bash -n`；本次将依次完成四卡一 epoch 训练和 RefCOCO、GroundingSuite、GRES 四指标、DLC-Bench（本地 Llama 3.1 8B）评测。



### 2026-09-08 - 固化干净 direct-GRPO 对照的四 bench 自动评测
