...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto, do_optimizer_step: bool = True, grad_weight: float = 1.0) -> dict[str, Any]:
        """Update the actor policy.
        
        Args:
            data: DataProto containing the batch data
            do_optimizer_step: If True, perform optimizer step after computing gradients.
                              If False, only accumulate gradients (for gradient accumulation).
            grad_weight: Weight to scale the loss for gradient accumulation (e.g., 0.5 for 2 batches).
        """
        ...
        global_batch_size = data.meta_info.get('global_batch_size_per_device', self.config.global_batch_size_per_device)
        mini_batches = data.select(select_keys, non_tensor_select_keys).split(global_batch_size)
    ...
    def optimizer_step(self) -> dict[str, Any]:
        """Perform optimizer step. Call this after accumulating gradients."""
        metrics = {}
        grad_norm = self._optimizer_step()
    ...
    def update_supervised(
        ...
        for micro_batch in micro_batches:
            ...
            token_weight = model_inputs.get("token_weight")
            if token_weight is not None:
                token_weight = token_weight.to(dtype=torch.float32)
            else:
                token_weight = torch.ones_like(response_mask)
            log_probs = self._forward_micro_batch(model_inputs, temperature=1.0)
            weighted_mask = response_mask * sample_weight * token_weight
