...
def refusal_credit_loss_weight(
    step: int,
    target_weight: float,
    warmup_start_step: int,
    warmup_end_step: int,
) -> float:
    """Return the explicit no-target refusal CE weight."""
    return direct_grounding_loss_weight(step, target_weight, warmup_start_step, warmup_end_step)
...
@dataclass
class RefusalCreditConfig:
    """Teacher-force only the no-target answer as an explicit refusal credit."""

    enabled: bool = False
    loss_weight: float = 0.05
    include_positive_sources: bool = False
    include_no_target: bool = True
    warmup_start_step: int = 0
    warmup_end_step: int = 0
    ...
    def post_init(self):
        if self.loss_weight < 0:
            raise ValueError("refusal_credit.loss_weight must be non-negative.")
        if self.warmup_start_step < 0 or self.warmup_end_step < self.warmup_start_step:
            raise ValueError(
                "refusal_credit warmup steps must satisfy 0 <= warmup_start_step <= warmup_end_step."
            )
        if self.enabled and not self.include_no_target:
            raise ValueError("refusal_credit requires include_no_target=true.")
