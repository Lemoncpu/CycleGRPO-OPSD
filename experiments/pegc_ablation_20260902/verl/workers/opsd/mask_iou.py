...
NO_TARGET_REFUSAL_PATTERN = re.compile(r"\bno\s+target\.", re.IGNORECASE)
...
def hierarchical_mask_token_weights(
    ...
    *,
    ...
    """Weight SAMTok code depths separately for teacher-forcing CE."""
    ...
    for row in range(response_ids.shape[0]):
        positions = torch.where(active[row] & (response_ids[row] >= mask_token_start) & (response_ids[row] <= mask_token_end))[0]
