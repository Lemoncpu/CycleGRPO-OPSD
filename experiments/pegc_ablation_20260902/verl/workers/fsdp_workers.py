...
class FSDPWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def accumulate_refusal_credit_gradients(self, data: DataProto):
        """Accumulate explicit no-target refusal teacher-forcing credit."""
        assert self._has_actor
        grad_weight = data.meta_info.get("grad_weight", 1.0)
        self._process_multi_modal_inputs(data)
        data = data.to(torch.cuda.current_device())
        ...
        with self.ulysses_sharding_manager:
            ...
            metrics = self.actor.update_supervised(
                data=data,
                grad_weight=grad_weight,
                metric_name="supervised_anchors/refusal_credit_loss",
            )
            output = DataProto(
                non_tensor_batch={
                    key: np.array([value] if np.isscalar(value) else value)
