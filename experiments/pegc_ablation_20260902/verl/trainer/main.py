...
class Runner:
    ...
    def run(self, config: PPOConfig):
        ...
        if (
            ...
            or config.worker.supervised_anchors.direct_mask_ce.enabled
            or config.worker.supervised_anchors.refusal_credit.enabled
        ):
...
def main():
    ...
    if not ray.is_initialized():
        ...
        local_num_cpus = os.environ.get("RAY_LOCAL_NUM_CPUS")
        if local_num_cpus:
            try:
                local_num_cpus = int(local_num_cpus)
            except ValueError as exc:
                raise ValueError("RAY_LOCAL_NUM_CPUS must be a positive integer") from exc
            if local_num_cpus < 1:
                raise ValueError("RAY_LOCAL_NUM_CPUS must be a positive integer")
            ray_init_kwargs["num_cpus"] = local_num_cpus
            print(f"Local Ray CPU resource cap: {local_num_cpus}")
