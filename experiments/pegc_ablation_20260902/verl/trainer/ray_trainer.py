...
class RayPPOTrainer:
    ...
    def _make_seg_batch_data_for_caption(
        ...
        configured_prompt_variants = localization_prompt_variants(
            len(batch), self.config.worker.opsd.pixel_iou.localization_prompt_mode
        )
        for i in range(len(batch.non_tensor_batch[seg_media_key])):
            ...
            prompt_variant = (
                ...
                else configured_prompt_variants[i]
            )
    ...
    def _make_direct_grounding_batch(
        ...
        _, direct_batch = self._make_seg_batch_data_for_caption(
            ...
            localization_prompt_variant_overrides=localization_prompt_variants(
                len(queries), self.config.worker.opsd.pixel_iou.localization_prompt_mode
            ),
            dataset=dataset,
        )
    ...
    def _make_direct_mask_ce_batch(
        ...
        """
        config = config or self.config.worker.supervised_anchors.direct_mask_ce
        if not config.enabled or "grounding_query" not in cycle_batch.non_tensor_batch:
            return None
        ...
        selected_uids = []
        selected_sources = []
        prompt_variants = localization_prompt_variants(
            len(indices), self.config.worker.opsd.pixel_iou.localization_prompt_mode
        )
        ...
        return DataProto.from_dict(
            tensors={
                ...
                "token_weight": hierarchical_mask_token_weights(
                    responses, response_masks,
                    coarse_weight=self.config.worker.opsd.mask_credit.coarse_weight,
                    fine_weight=self.config.worker.opsd.mask_credit.fine_weight,
                ) if self.config.worker.opsd.mask_credit.enabled else torch.ones_like(responses, dtype=torch.float32),
            },
            non_tensors={
    ...
    def _build_distillation_batch(
        ...
        evidence_config = self.config.worker.opsd.evidence_gate
        if evidence_config.enabled:
            gates = torch.tensor(
                [evidence_gate_weight(caption_batch.non_tensor_batch["privileged_context"][index],
                                      min_weight=evidence_config.min_weight,
                                      max_weight=evidence_config.max_weight,
                                      false_positive_penalty=evidence_config.false_positive_penalty)
                 for index in mid_indices], dtype=torch.float32
            )
            weights = weights * gates
            distill_metrics["opsd/evidence_gate_mean"] = float(gates.mean())
            distill_metrics["opsd/evidence_gate_active_count"] = int(gates.numel())
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                refusal_credit_batch = None
                caption_qa_batch = None
                cbba_cycle_scale = 1.0
                cbba_supervised_scale = 1.0
                ...
                if direct_parent_batch is not None:
                    ...
                    refusal_credit_config = self.config.worker.supervised_anchors.refusal_credit
                    ...
                    if (
                        ...
                        or (
                            refusal_credit_config.enabled
                            and refusal_credit_config.include_positive_sources
                        )
                    ):
                    ...
                    if (
                        ...
                        or (
                            refusal_credit_config.enabled
                            and refusal_credit_config.include_no_target
                        )
                    ):
                ...
                if cycle_batch is not None and (
                    ...
                    with timer("gen", timing_raw):
                        ...
                        if self.config.worker.opsd.enabled and self.config.worker.opsd.pixel_iou.enabled:
                            ...
                            cbba_config = self.config.worker.opsd.cbba
                            if cbba_config.enabled:
                                (
                                    cbba_cycle_scale,
                                    cbba_supervised_scale,
                                    cbba_confidence,
                                ) = cbba_stream_scales(
                                    caption_scores.tolist(),
                                    target_confidence=cbba_config.target_confidence,
                                    gain=cbba_config.gain,
                                    min_cycle_scale=cbba_config.min_cycle_scale,
                                    max_cycle_scale=cbba_config.max_cycle_scale,
                                    min_supervised_scale=cbba_config.min_supervised_scale,
                                    max_supervised_scale=cbba_config.max_supervised_scale,
                                )
                                metrics.update({
                                    "opsd/cbba_cycle_confidence": cbba_confidence,
                                    "opsd/cbba_cycle_scale": cbba_cycle_scale,
                                    "opsd/cbba_supervised_scale": cbba_supervised_scale,
                                })
                ...
                if direct_parent_batch is not None:
                    ...
                    refusal_credit_batch = self._make_direct_mask_ce_batch(
                        direct_parent_batch,
                    ...
                    if refusal_credit_batch is not None:
                        refusal_sources = np.asarray(
                            refusal_credit_batch.non_tensor_batch["source"], dtype=object
                        )
                        metrics["supervised_anchors/refusal_credit_samples"] = int(
                            np.sum(refusal_sources == "supervised_grounding_no_target")
                        )
                    if (
                        ...
                        no_target_groups = defaultdict(list)
                        direct_uids = np.asarray(
                            direct_grounding_batch.non_tensor_batch.get("sample_uid", []), dtype=object
                        )
                        direct_mask_counts = np.asarray(
                            direct_grounding_batch.non_tensor_batch.get("mask_group_count", []), dtype=float
                        )
                        for uid, source, count in zip(
                            direct_uids, direct_rollout_sources, direct_mask_counts
                        ):
                            if source == "supervised_grounding_no_target":
                                no_target_groups[str(uid)].append(bool(count > 0))
                        expected_rollouts = self.config.worker.supervised_anchors.direct_grounding.rollouts
                        complete_groups = [
                            values for values in no_target_groups.values()
                            if len(values) == expected_rollouts
                        ]
                        if complete_groups:
                            all_mask = [all(values) for values in complete_groups]
                            all_empty = [not any(values) for values in complete_groups]
                            homogeneous = [len(set(values)) == 1 for values in complete_groups]
                            metrics.update(
                                {
                                    "supervised_anchors/no_target_prompt_groups": len(complete_groups),
                                    "supervised_anchors/no_target_all_6_mask_rate": float(np.mean(all_mask)),
                                    "supervised_anchors/no_target_all_6_empty_rate": float(np.mean(all_empty)),
                                    "supervised_anchors/no_target_6_rollout_homogeneous_rate": float(np.mean(homogeneous)),
                                }
                            )
                        else:
                            metrics.update(
                                {
                                    "supervised_anchors/no_target_prompt_groups": 0,
                                    "supervised_anchors/no_target_all_6_mask_rate": 0.0,
                                    "supervised_anchors/no_target_all_6_empty_rate": 0.0,
                ...
                if self.config.worker.actor.optimize_segmenter and self.config.worker.actor.optimize_captioner:
                    ...
                    refusal_credit_size = len(refusal_credit_batch) if refusal_credit_batch is not None else 0
                    ...
                    balance_config = self.config.worker.opsd.adaptive_balance
                    if balance_config.enabled:
                        stream_sizes = [size for size in (cap_batch_size, seg_batch_size, direct_grounding_size, caption_qa_size) if size > 0]
                        reference = float(np.mean(stream_sizes)) if stream_sizes else 1.0
                        def _scale(size: int) -> float:
                            if size <= 0: return 1.0
                            return float(np.clip(np.sqrt(reference / size), balance_config.min_scale, balance_config.max_scale))
                        cap_grad_weight *= _scale(cap_batch_size)
                        cycle_seg_grad_weight *= _scale(seg_batch_size)
                        metrics["opsd/adaptive_balance_caption_scale"] = _scale(cap_batch_size)
                        metrics["opsd/adaptive_balance_segmentation_scale"] = _scale(seg_batch_size)
                    cycle_seg_grad_weight *= cbba_cycle_scale
                    ...
                    refusal_credit_weight = (
                        refusal_credit_loss_weight(
                            self.global_step,
                            refusal_credit_config.loss_weight,
                            refusal_credit_config.warmup_start_step,
                            refusal_credit_config.warmup_end_step,
                        )
                        if refusal_credit_config.enabled
                        else 0.0
                    )
                    caption_qa_grad_weight = (
                        self.config.worker.supervised_anchors.caption_qa.loss_weight
                        if self.config.worker.supervised_anchors.caption_qa.enabled else 0.0
                    )
                    if balance_config.enabled:
                        direct_grad_weight *= _scale(direct_grounding_size)
                        direct_ce_weight *= _scale(direct_mask_ce_size)
                        refusal_credit_weight *= _scale(refusal_credit_size)
                        caption_qa_grad_weight *= _scale(caption_qa_size)
                        metrics["opsd/adaptive_balance_direct_scale"] = _scale(direct_grounding_size)
                        metrics["opsd/adaptive_balance_refusal_credit_scale"] = _scale(refusal_credit_size)
                        metrics["opsd/adaptive_balance_qa_scale"] = _scale(caption_qa_size)
                    direct_grad_weight *= cbba_supervised_scale
                    direct_ce_weight *= cbba_supervised_scale
                    refusal_credit_weight *= cbba_supervised_scale
                    caption_qa_grad_weight *= cbba_supervised_scale
                    metrics.update(
                        {
                            ...
                            "supervised_anchors/refusal_credit_weight": (
                                refusal_credit_config.loss_weight if refusal_credit_config.enabled else 0.0
                            ),
                            "supervised_anchors/refusal_credit_weight_effective": refusal_credit_weight,
                            "supervised_anchors/refusal_credit_samples": refusal_credit_size,
                            "supervised_anchors/caption_qa_loss_weight": caption_qa_grad_weight,
                    ...
                    if self.config.trainer.critic_warmup <= self.global_step:
                        ...
                        with timer("update_actor", timing_raw):
                            ...
                            if refusal_credit_batch is not None and refusal_credit_size > 0:
                                refusal_batch, refusal_pad = pad_dataproto_to_divisor(
                                    refusal_credit_batch,
                                    self.actor_rollout_ref_wg.world_size
                                    * self.config.worker.actor.micro_batch_size_per_device_for_update,
                                )
                                if refusal_pad:
                                    refusal_batch.batch["sample_weight"][-refusal_pad:] = 0.0
                                self._balance_batch(
                                    refusal_batch,
                                    metrics=metrics,
                                    logging_prefix="refusal_credit_seqlen",
                                )
                                refusal_batch.meta_info["global_token_num"] = torch.sum(
                                    refusal_batch.batch["attention_mask"], dim=-1
                                ).tolist()
                                refusal_batch.meta_info["grad_weight"] = refusal_credit_weight
                                refusal_batch.meta_info["global_batch_size_per_device"] = (
                                    len(refusal_batch) // self.actor_rollout_ref_wg.world_size
                                )
                                refusal_output = self.actor_rollout_ref_wg.accumulate_refusal_credit_gradients(
                                    refusal_batch
                                )
                                actor_metrics.update(reduce_metrics(refusal_output.non_tensor_batch))
                                capture_multitask_gradient_component("refusal_credit")
