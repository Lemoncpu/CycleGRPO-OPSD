...
# 1/10, one-epoch Evidence+Mask Credit with continuous pixel-empty no-target reward.
# Current host default: GPU0-2 train and GPU3 runs the DLC judge; set TRAIN_GPU_LIST, TRAIN_NUM_GPUS, LLAMA_GPU and batch vars for 8 GPUs.
...
RAY_SHORT_ROOT="${RAY_SHORT_ROOT:-/dev/shm/pegc-e-pixel-empty-iou}"
RUN_ROOT="${REPO_DIR}/logs/pegc_evidence_mask_credit_pixel_empty_iou_1of10_epoch1"
...
if ! curl --silent --max-time 3 -H "Authorization: Bearer ${LLAMA_API_KEY}" "http://127.0.0.1:${LLAMA_PORT}/v1/models" | grep -q "\"${LLAMA_MODEL_NAME}\""; then
  echo "Starting local Llama judge on physical GPU${LLAMA_GPU}..."
...
CAPTION_QA_BATCH_SIZE="${CAPTION_QA_BATCH_SIZE}" \
...
NO_TARGET_REWARD_MODE="${NO_TARGET_REWARD_MODE:-pixel_empty_iou}" \
...
SAVE_FREQ=18 \
SAVE_LIMIT=3 \
RUN_NAME=pegc_evidence_mask_credit_pixel_empty_iou_1of10_epoch1 \
RUN_ROOT="${REPO_DIR}/logs/pegc_evidence_mask_credit_pixel_empty_iou_1of10_epoch1" \
