...
# 1/10, one-epoch direct GRPO + hierarchical Mask Credit only.
# Positive supervision: direct GRPO plus GT-mask CE with hierarchical token credit.
# No-target supervision remains direct GRPO only so this run isolates Mask Credit.
# GPU0-3 train; no Llama service is needed because DLC-QA is disabled.
