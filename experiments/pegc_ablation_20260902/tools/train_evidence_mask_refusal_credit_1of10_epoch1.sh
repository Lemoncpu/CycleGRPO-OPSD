...
# 1/10, one-epoch Evidence + Mask Credit + Refusal Credit.
# Default 8-GPU entry point: GPU0-6 train and GPU7 runs the DLC judge; current 4-GPU host overrides this to 3+1 at launch.
...
RAY_SHORT_ROOT="${RAY_SHORT_ROOT:-/dev/shm/pegc-e-r0123}"
RUN_ROOT="${REPO_DIR}/logs/pegc_evidence_mask_refusal_credit_1of10_epoch1"
LLAMA_LOG="${LLAMA_LOG:-${RUN_ROOT}/llama_judge.log}"
LLAMA_PID=""
LLAMA_STARTED=false
# Use the project environment directly; conda activation is broken in the base shell on this host.
...
SAVE_FREQ=18 \
SAVE_LIMIT=3 \
RUN_NAME=pegc_evidence_mask_refusal_credit_1of10_epoch1 \
RUN_ROOT="${REPO_DIR}/logs/pegc_evidence_mask_refusal_credit_1of10_epoch1" \
