...
# 1/10, one-epoch direct GRPO + hierarchical Mask Credit + Refusal Credit.
# Positive supervision: direct GRPO plus GT-mask CE with hierarchical token credit.
# No-target supervision: direct GRPO plus explicit No target. teacher-forcing.
...
RUN_ROOT="${REPO_DIR}/logs/pegc_direct_grpo_mask_refusal_credit_1of10_epoch1"
...
SAVE_FREQ=18 \
SAVE_LIMIT=3 \
RUN_NAME=pegc_direct_grpo_mask_refusal_credit_1of10_epoch1 \
RUN_ROOT="${RUN_ROOT}" \
