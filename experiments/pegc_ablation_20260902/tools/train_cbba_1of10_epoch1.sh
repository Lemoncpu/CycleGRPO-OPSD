...
# 1/10, one-epoch baseline + CBBA confidence balance.
# Self-contained 8-GPU entry point: GPU0-6 train and GPU7 runs the DLC judge.
...
LLAMA_GPU="${LLAMA_GPU:-7}"
RAY_SHORT_ROOT="${RAY_SHORT_ROOT:-/dev/shm/pegc-c-g0123456}"
RUN_ROOT="${REPO_DIR}/logs/pegc_cbba_1of10_epoch1"
...
if ! curl --silent --max-time 3 -H "Authorization: Bearer ${LLAMA_API_KEY}" "http://127.0.0.1:${LLAMA_PORT}/v1/models" | grep -q "\"${LLAMA_MODEL_NAME}\""; then
  echo "Starting local Llama judge on physical GPU7..."
...
ROLLOUT_BATCH_SIZE=112 \
ACTOR_GLOBAL_BATCH_SIZE=112 \
DIRECT_BATCH_SIZE=224 \
CAPTION_QA_BATCH_SIZE=56 \
...
MASK_CREDIT_ENABLED=false \
ADAPTIVE_BALANCE_ENABLED=false \
CBBA_ENABLED=true \
CBBA_TARGET_CONFIDENCE=0.70 \
CBBA_GAIN=1.0 \
CBBA_MIN_CYCLE_SCALE=0.75 \
CBBA_MAX_CYCLE_SCALE=1.25 \
CBBA_MIN_SUPERVISED_SCALE=0.75 \
CBBA_MAX_SUPERVISED_SCALE=1.50 \
MAX_STEPS="${MAX_STEPS:-17}" \
TOTAL_EPOCHS=1 \
SAVE_FREQ=18 \
SAVE_LIMIT=3 \
RUN_NAME=pegc_cbba_1of10_epoch1 \
RUN_ROOT="${REPO_DIR}/logs/pegc_cbba_1of10_epoch1" \
