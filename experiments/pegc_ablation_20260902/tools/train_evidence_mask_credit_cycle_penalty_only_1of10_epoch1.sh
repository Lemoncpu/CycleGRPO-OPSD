...
# 1/10, one-epoch Evidence+Mask Credit with cycle-only positive no-target penalty.
# Local 4-GPU entry point: GPU0-2 train and GPU3 runs the DLC judge.
...
RAY_SHORT_ROOT="${RAY_SHORT_ROOT:-/dev/shm/pegc-cycle}"
RUN_ROOT="${REPO_DIR}/logs/pegc_evidence_mask_credit_cycle_penalty_only_1of10_epoch1"
...
CYCLE_POSITIVE_EMPTY_MASK_PENALTY=1.0 \
...
SAVE_FREQ=18 \
SAVE_LIMIT=3 \
RUN_NAME=pegc_evidence_mask_credit_cycle_penalty_only_1of10_epoch1 \
RUN_ROOT="${REPO_DIR}/logs/pegc_evidence_mask_credit_cycle_penalty_only_1of10_epoch1" \
